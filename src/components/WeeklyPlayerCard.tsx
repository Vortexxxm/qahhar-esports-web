...
const WeeklyPlayerCard = ({ 
  ...
  return (
    <div className="relative group mb-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-yellow-500 via-orange-600 to-red-700 border-4 border-yellow-300 shadow-2xl transform hover:scale-105 transition-all duration-500 max-w-md mx-auto">
        ...
        <div className="absolute top-6 left-1/2 transform -translate-x-1/2 z-10">
          <Badge className="bg-gradient-to-r from-yellow-200 to-orange-200 text-orange-900 font-bold text-sm px-4 py-2 shadow-lg border-2 border-yellow-300 animate-pulse">
        ...
        <div className="relative pt-16 pb-6 px-6 text-center bg-gradient-to-b from-yellow-600/20 to-transparent">
          <div className="relative">
            ...
            <div className="relative mx-auto mb-4">
              ...
              <Avatar className="relative w-28 h-28 border-6 border-yellow-200 shadow-2xl mx-auto">
                <AvatarImage src={player.avatar_url || ""} />
                <AvatarFallback className="bg-gradient-to-br from-yellow-500 to-orange-600 text-yellow-100 text-3xl font-bold">
              ...
              <div className="absolute -inset-1 rounded-full">
                <Flame className="absolute top-0 left-1/4 w-6 h-6 text-orange-400 animate-bounce" />
                <Flame className="absolute top-1/4 right-0 w-5 h-5 text-red-400 animate-pulse delay-300" />
                <Flame className="absolute bottom-1/4 left-0 w-5 h-5 text-yellow-400 animate-bounce delay-500" />
                <Zap className="absolute bottom-0 right-1/4 w-6 h-6 text-yellow-300 animate-ping delay-700" />
              </div>
            </div>
            
            <h3 className="text-2xl font-bold text-yellow-100 mb-2 drop-shadow-lg">
              {player.username || player.full_name}
            </h3>
            
            <Badge className="bg-gradient-to-r from-yellow-300 to-orange-300 text-orange-900 font-bold text-base px-4 py-2 shadow-lg">
        ...
        <div className="px-6 pb-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gradient-to-br from-yellow-200/20 to-orange-200/20 backdrop-blur-sm border-2 border-yellow-300/50 rounded-xl p-4 text-center group/stat hover:scale-105 transition-transform duration-300">
              <Trophy className="h-6 w-6 text-yellow-200 mx-auto mb-2 group-hover/stat:animate-bounce" />
              <div className="text-2xl font-bold text-yellow-100">{stats.wins}</div>
              <div className="text-xs text-yellow-200 font-medium">انتصارات</div>
            </div>
            
            <div className="bg-gradient-to-br from-orange-200/20 to-red-200/20 backdrop-blur-sm border-2 border-orange-300/50 rounded-xl p-4 text-center group/stat hover:scale-105 transition-transform duration-300">
              <Target className="h-6 w-6 text-orange-200 mx-auto mb-2 group-hover/stat:animate-bounce" />
              <div className="text-2xl font-bold text-orange-100">{getKDRatio(stats.kills, stats.deaths)}</div>
              <div className="text-xs text-orange-200 font-medium">K/D</div>
            </div>
          </div>

          <div className="bg-gradient-to-br from-red-200/20 to-yellow-200/20 backdrop-blur-sm border-2 border-red-300/50 rounded-xl p-4 text-center group/stat hover:scale-105 transition-transform duration-300">
            <Gamepad2 className="h-6 w-6 text-red-200 mx-auto mb-2 group-hover/stat:animate-bounce" />
            <div className="text-3xl font-bold text-red-100">{stats.points}</div>
            <div className="text-xs text-red-200 font-medium">النقاط الإجمالية</div>
          </div>

          {/* Bio Section */}
          {player.bio && (
            <div className="bg-gradient-to-br from-black/30 to-yellow-900/20 backdrop-blur-sm border-2 border-yellow-400/30 rounded-xl p-4">
              <p className="text-yellow-100 text-sm text-center font-medium">{player.bio}</p>
            </div>
          )}
          ...
          <div className="flex justify-center pt-2">
            <Button
              ...
              } rounded-xl px-6 py-3 border-2 border-yellow-400/50 hover:border-yellow-300/70`}
            >
              ...
              <div className="relative flex items-center space-x-3 rtl:space-x-reverse">
                <Heart className={`h-6 w-6 transition-all duration-300 ${isLiked ? 'fill-current scale-110 text-red-300' : 'group-hover/btn:scale-110'}`} />
                <span className="font-bold text-xl">{likesCount}</span>
      ...
      <div className="absolute -top-4 -left-4 w-8 h-8 bg-yellow-400 rounded-full opacity-60 animate-bounce delay-200"></div>
      <div className="absolute -top-2 -right-6 w-6 h-6 bg-orange-400 rounded-full opacity-50 animate-ping delay-500"></div>
      <div className="absolute -bottom-4 -right-4 w-7 h-7 bg-red-400 rounded-full opacity-60 animate-pulse delay-800"></div>
      <div className="absolute -bottom-2 -left-6 w-5 h-5 bg-yellow-500 rounded-full opacity-50 animate-bounce delay-1000"></div>
    </div>
  );
};
